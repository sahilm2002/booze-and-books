...
DROP FUNCTION IF EXISTS transfer_book_ownership_on_completion_debug_028();

CREATE OR REPLACE FUNCTION transfer_book_ownership_on_completion_debug_028()
RETURNS TRIGGER AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trigger_transfer_ownership_on_completion_debug_028 ON swap_requests;
CREATE TRIGGER trigger_transfer_ownership_on_completion_debug_028
    AFTER UPDATE ON swap_requests
    FOR EACH ROW
    WHEN (NEW.status = 'COMPLETED')
    EXECUTE FUNCTION transfer_book_ownership_on_completion_debug_028();

-- Grant execute permissions
GRANT EXECUTE ON FUNCTION transfer_book_ownership_on_completion_debug_028() TO authenticated;
